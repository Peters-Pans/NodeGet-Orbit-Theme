...
默认使用 ORBIT，页面顶部可随时切换 VECTOR / ORBIT，刷新后保留选择。链接中的 `?design=vector` 或 `?design=orbit` 可指定主题；未指定时使用本地偏好或仓库默认主题。资源曲线展示本次会话的最近 60 个采样点。延迟历史支持 1 小时、6 小时、24 小时与 7 天窗口，沿用原有查询上限与刷新频率。
